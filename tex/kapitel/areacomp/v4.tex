...
\begin{algorithm}
    \KwData {$from, to$ Start- und Endindizes}
    \KwResult{ $current \leftarrow floorInterval(from)$}
    \While {$current \neq \texttt{null}$} {
...
\begin{algorithm}
    \KwData{$from, to: $ Start- und Endindizes, beide inklusiv,\\$intervalIndex: $ Die Instanz der \texttt{RuleIntervalIndex} Datenstruktur}
...
\begin{algorithm}
    \KwData{ $positions: $ Array von Indizes an denen $p$ vorkommt, $len: $ Länge von $p$}
    $count \leftarrow 0$\;
...
\begin{algorithm}
    \KwData{ $positions: $ Array von Indizes an denen $p$ vorkommt, entfernte Positionen $=-1$}
...
\begin{algorithm}
        \KwData{ $len: $ Länge des Musters,\\ $positions: $ Array mit Startindizes der Vorkommen. Entfernte Vorkommen sind $-1$,\\ $intervalIndex: $ Die Instanz der \texttt{RuleIntervalIndex} Datenstruktur}
