...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\matrix (A) [matrix of nodes, nodes={draw, minimum size=10mm, anchor=center}, column sep=-\pgflinewidth]{
			a & b & c & a & b & c & d\\
		};
		
		\begin{scope}[yshift=-2cm]
			\matrix (B) [matrix of nodes, nodes={draw, minimum size=10mm, anchor=center}, nodes in empty cells, column sep=-\pgflinewidth]{
				a & A &  & a & A & & d\\
			};
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\node[draw=none, below=3cm of Symbols] (Queue) {Warteschlange};
		
		\matrix (A) [matrix of nodes, nodes={draw, minimum size=10mm, anchor=center}, column sep=-\pgflinewidth, right=1cm of Symbols]{
			a & b & c & a & b & c & d\\
		};
		
		\matrix (lists) [matrix of nodes, nodes={minimum size=10mm, anchor=center}, row sep=5mm, column sep=1cm, right=1cm of Queue, nodes in empty cells]{
			& |[draw]| bc & \\
			& |[draw]| ab & \\
			$>3$ & $2$ & $3$ \\
		};
		...
		\matrix (hash) [matrix of nodes, nodes={draw, minimum size=10mm, anchor=center}, column sep=-\pgflinewidth, right=1cm of lists-2-3]{
			bc\\
			ab\\
		};
	...
	\end{tikzpicture}
	\caption{Die Datenstrukturen von RePair}
\end{figure}

\subsection{Beispiel}

Als Beispiel wenden wir nun den Re-Pair Algorithmus auf $abcabcd$ an.
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption*{Die Datenstrukturen von RePair zu Anfang}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\matrix (A) [matrix of nodes, nodes={draw, minimum size=10mm, anchor=center}, nodes in empty cells, column sep=-\pgflinewidth, right=1cm of Symbols]{
			A & & c & A & & c & d\\
		};
	...
	\end{tikzpicture}
	\caption*{Die Datenstrukturen nach Ersetzung von $ab$ durch $A$}
\end{figure}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption*{Die Datenstrukturen nach Ersetzung von $Ac$ durch $B$}
\end{figure}

Die Prioritätswarteschlange ist nun leer und damit terminiert der Algorithmus. Die resultierende Grammatik ist:
